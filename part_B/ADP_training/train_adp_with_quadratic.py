...
FEATURE_COLS = [
    ...
    "low_override_r1",
    "low_override_r2",
    "T1_sq",           # T1² — becomes quadratic in p1 through dynamics at decision time
    "T2_sq",           # T2² — becomes quadratic in p2 through dynamics at decision time
]
...
def train(samples_path="samples_dummy.csv", output_path="eta_dummy_with_quadratic.pkl", ridge_lambda=10.0):
    """
    ...
    # Compute quadratic features from raw state columns
    # At decision time T1_sq → T1_next² = (alpha + ef*p1)² → quadratic in p1
    df["T1_sq"] = df["T1"] ** 2
    df["T2_sq"] = df["T2"] ** 2
...
if __name__ == "__main__":
    ...
    samples_path = (sys.argv[1] if len(sys.argv) > 1
                    else os.path.join(base_dir, "samples_dummy.csv"))
    ridge_lambda = float(sys.argv[2]) if len(sys.argv) > 2 else 10.0
    output_path  = os.path.join(base_dir, "eta_dummy_with_quadratic.pkl")
