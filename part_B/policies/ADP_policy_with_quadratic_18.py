...
# ─────────────────────────────────────────────
_ETA_PATH = os.path.join(_PART_B_DIR, "ADP_training", "eta_dummy_with_quadratic.pkl")
...
def _solve_milp(state, data, eta_next, prices, occs, price_mean, occ1_mean, occ2_mean):
    ...
    # Quadratic expressions — become quadratic in p1, p2 through dynamics:
    #   T1_next = alpha + ef*p1 - hv*v  →  T1_next² contains ef²*p1² (quadratic in p1)
    # This requires Gurobi NonConvex=2 due to p1*v bilinear term
    T1_next_sq = T1_next ** 2
    T2_next_sq = T2_next ** 2
    ...
    # Note: bias is now at index 12 (shifted by T1_sq at 10, T2_sq at 11)
    vf_const = (  eta_next[3]  * occ1_mean
                ...
                + eta_next[12]                     # bias (index shifted)
               )

    # VF — Pyomo expression terms (linear and quadratic in decision variables)
    # eta[10]*T1_next_sq and eta[11]*T2_next_sq introduce quadratic terms in p1, p2
    vf_linear = (  eta_next[0]  * T1_next
                 + eta_next[1]  * T2_next
                 + eta_next[2]  * H_next
                 + eta_next[7]  * vc_next
                 + eta_next[10] * T1_next_sq   # quadratic in p1
                 + eta_next[11] * T2_next_sq   # quadratic in p2
                )
    ...
    solver.options["NonConvex"]   = 2   # needed for bilinear p1*v term in T1_next²
    solver.solve(m)
...
def select_action(state):
    ...
    # Size derived from ETA so it stays correct if features change
    _n_eta   = len(next(iter(ETA.values())))
    eta_next = ETA.get(t + 1, np.zeros(_n_eta))
